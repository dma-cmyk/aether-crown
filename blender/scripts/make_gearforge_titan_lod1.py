...
def build_foot_lod1(sx, H, Tt, C):
    ...
    x = 1.58 * sx
    T.box(f"{ASSET}_sole_{side}", (1.62, 2.55, 0.30), (x, 0.05, 0.18), H, bevel=0.04)
    ...
    T.box(f"{ASSET}_shin_{side}", (0.94, 1.08, 2.05), (x, 0.20, 2.02), H,
          rot=(math.radians(-5.0), 0.0, 0.0), bevel=0.0)
    T.trapezoid(f"{ASSET}_shin_armor_{side}", (1.34, 0.54), (1.05, 0.42), 1.75,
                (x, -0.57, 2.13), H, bevel=0.0)
    T.cylinder(f"{ASSET}_knee_{side}", 0.58, 1.42, (x, -0.16, 3.08), Tt,
               vertices=14, rot=(0.0, math.radians(90.0), 0.0), bevel=0.0)
    T.box(f"{ASSET}_thigh_{side}", (1.12, 1.26, 1.58), (x, 0.26, 3.88), H,
          rot=(math.radians(7.0), 0.0, 0.0), bevel=0.0)
    T.box(f"{ASSET}_thigh_plate_{side}", (1.30, 0.36, 1.18), (x, -0.53, 3.92), H, bevel=0.0)
    ...
    for offset in (-0.38, 0.38):
        T.cylinder_between(
            ...
            0.105, C, vertices=14, bevel=0.0)
        T.cylinder_between(
            f"{ASSET}_thigh_piston_{side}_{int((offset + 0.5) * 10)}",
            (x + offset, 0.66, 3.02), (x + offset, 0.78, 4.58),
            0.095, Tt, vertices=14, bevel=0.0)


def build_body_lod1(H, Tt, A):
...
def build_reactor_lod1(H, Tt, C, A, F):
    ...
    for sx in (-1.0, 1.0):
        ...
        T.cylinder_between(f"{ASSET}_reactor_pipe_a_{side}", (sx * 0.60, 2.12, 7.18),
                           (sx * 1.18, 1.90, 8.04), 0.115, C, vertices=14, bevel=0.0)
        T.cylinder_between(f"{ASSET}_reactor_pipe_b_{side}", (sx * 1.18, 1.90, 8.04),
                           (sx * 1.18, 1.62, 8.78), 0.115, C, vertices=14, bevel=0.0)
