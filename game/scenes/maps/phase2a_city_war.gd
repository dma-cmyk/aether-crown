extends Node3D
## Phase 2A city-war map: Player HQ (NW) vs Enemy HQ (SE) + 3 neutral cities
## (West player-side, Central contested middle, East enemy-side).
## Reuses Phase 1 / 1.5 systems untouched: camera, selection, orders, unit
## combat, economy, production, population, enemy strategy AI (extended for
## cities), victory/defeat/restart. Cities grant Material/sec to the owner.
## No Territory, no Aether, no districts, no city production (Phase 2B+).
...
func _ready() -> void:
	...
	print("CITYWAR_READY p_hq=%s e_hq=%s cities=%d" % [str(player_hq.global_position), str(enemy_hq.global_position), cities.size()])
	...
	for o in get_tree().get_nodes_in_group("rts_units"):
		...
		if u != null and u.is_alive():
			if u.is_player:
				...
				e += 1
	var counts := city_counts()
	print("CITYWAR_PERF t=%s fps=%d p=%d e=%d kills_p=%d kills_e=%d mat_p=%d mat_e=%d ai=%s cities=P%d/N%d/E%d" % [
		...
		int(counts["player"]), int(counts["neutral"]), int(counts["enemy"]),
	])
...
func _on_game_over(player_won: bool) -> void:
	print("CITYWAR_OVER victory=" if player_won else "CITYWAR_OVER defeat=", player_won)
	_hud().call("show_end", player_won)


func _on_city_owner_changed(new_owner: int, city: RTSCity) -> void:
	_recalc_city_bonus()
	var who := city.owner_name()
	_hud().call("toast", "%s captured: %s (+%.1f/s)" % [city.display_name, who, city.income_bonus])
...
func _recalc_city_bonus() -> void:
	...
	print("CITYWAR bonus_p=%.1f bonus_e=%.1f" % [bonus_p, bonus_e])
