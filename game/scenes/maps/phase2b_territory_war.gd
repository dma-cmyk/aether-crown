extends Node3D
## Phase 2B territory-war map: Phase 2A city war + 5 territory regions whose
## ownership follows city ownership, granting Aether/sec to the owner.
## Territory ownership changes ONLY via city capture (walking into a region
## never flips it). Base territories are fixed. Victory/defeat still come
## from HQ destruction only. No districts, no Aether spending (Phase 2C+).
...
func _ready() -> void:
	...
	print("TERRITORY_WAR_READY p_hq=%s e_hq=%s cities=%d territories=%d" % [str(player_hq.global_position), str(enemy_hq.global_position), cities.size(), territories.regions.size()])
...
func _on_territories_changed() -> void:
	...
	for r in territories.regions:
		...
		if old != region.owner_side:
			_territory_owners[region.region_id] = region.owner_side
			print("TERRITORY_SHIFT %s -> %s aether=%.2f/s" % [region.display_name, region.owner_name(), region.definition.aether_income_bonus])
			_hud().call("toast", "%s: %s (Aether +%.2f/s)" % [region.display_name, region.owner_name(), region.definition.aether_income_bonus])


func _apply_territory_income() -> void:
	economy_p.bonus_aether = territories.aether_for(true)
	economy_e.bonus_aether = territories.aether_for(false)
...
func _log_perf() -> void:
	...
	print("TERRITORY_PERF t=%s fps=%d p=%d e=%d kills_p=%d kills_e=%d mat_p=%d mat_e=%d ae_p=%.1f ae_e=%.1f ai=%s cities=P%d/N%d/E%d terr=P%d/N%d/E%d" % [
...
func _recalc_city_bonus() -> void:
	...
	for c in cities:
		...
		if city.owner_side == RTSCity.Owner.PLAYER:
			bonus_p += city.income_bonus
		elif city.owner_side == RTSCity.Owner.ENEMY:
			bonus_e += city.income_bonus
	economy_p.bonus_income = bonus_p
	economy_e.bonus_income = bonus_e
	print("TERRITORY_CITYWAR bonus_p=%.1f bonus_e=%.1f" % [bonus_p, bonus_e])
