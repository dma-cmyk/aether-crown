extends CanvasLayer
## Phase 1.5 match HUD: economy/pop/time top bar, outpost status, selection
## panel with HQ production, end-screen overlay with Restart. Debug numbers
## (FPS/counts/AI) stay in a separate dimmed box.
...
@onready var _outpost_label: Label = $OutpostBox/OutpostLabel
...
func setup(map_node: Node3D) -> void:
	...
	_outpost = map_node.get("outpost") as RTSOutpost
...
func _refresh_outpost() -> void:
	var text := "OUTPOST: %s" % _outpost.owner_name()
	if _outpost.contested:
		text += " CONTESTED"
	elif _outpost.challenger >= 0:
		text += " %d%%" % int(_outpost.progress_fraction() * 100.0)
	_outpost_label.text = text
