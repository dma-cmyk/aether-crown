extends CanvasLayer
## Phase 2C district-war HUD: phase2b_hud plus a compact City Management
## panel (city buttons + slot status + build buttons). City selection for
## district building is HUD-only: the RTS world-click selection is untouched.
...
var _mgmt_city: String = "west_city"
...
func _ready() -> void:
	...
	($DistrictBox/CityRow/BtnWest as Button).pressed.connect(_mgmt_select.bind("west_city"))
	($DistrictBox/CityRow/BtnCentral as Button).pressed.connect(_mgmt_select.bind("central_city"))
	($DistrictBox/CityRow/BtnEast as Button).pressed.connect(_mgmt_select.bind("east_city"))
	($DistrictBox/BuildRow/BtnIndustry as Button).pressed.connect(_mgmt_build.bind("industry"))
	($DistrictBox/BuildRow/BtnMilitary as Button).pressed.connect(_mgmt_build.bind("military"))
	($DistrictBox/BuildRow/BtnAether as Button).pressed.connect(_mgmt_build.bind("aether_works"))
...
func setup(map_node: Node3D) -> void:
	...
	($DistrictBox/BuildRow/BtnIndustry as Button).text = "Industry (%dM/%dA)" % [ind.material_cost, ind.aether_cost]
	($DistrictBox/BuildRow/BtnMilitary as Button).text = "Military (%dM/%dA)" % [mil.material_cost, mil.aether_cost]
	($DistrictBox/BuildRow/BtnAether as Button).text = "Aether (%dM/%dA)" % [aew.material_cost, aew.aether_cost]
	if _sel != null:
